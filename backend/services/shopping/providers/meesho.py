...
class MeeshoProvider(ShoppingProvider):
    ...
    def is_configured(self) -> bool:
        return bool(os.getenv("MEESHO_API_KEY", "").strip()) or bool(os.getenv("TAVILY_API_KEY", "").strip()) or bool(os.getenv("GEMINI_API_KEY", "").strip())
    ...
    def search_products(self, query: str, filters: dict = None) -> dict:
        ...
        search_query = f"site:meesho.com {query}"
        ...
        try:
            ...
            for item in raw_results:
                ...
                prod_id_match = re.search(r'/p/([a-zA-Z0-9]+)', item_url)
                prod_id = prod_id_match.group(1) if prod_id_match else item_url.split("/")[-1]

                results.append({
                    "provider": self.name,
                    "provider_product_id": prod_id,
                    "title": title.replace(" - Buy ... online", "").split("on Meesho")[0].strip(),
                    "price": price,
                    "currency": "INR",
