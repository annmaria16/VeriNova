...
interface AuthContextType {
  ...
  oauthLogin: (provider: string, email: string, name: string, code?: string) => Promise<void>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      // Fetch user profile
      const profileRes = await api.get<UserProfile>("/user/profile");
  ...
  const oauthLogin = async (provider: string, email: string, fullname: string, code?: string) => {
    setError(null);
    try {
      const res = await api.post<{ access_token: string; token_type: string }>("/auth/oauth", {
        provider,
        email,
        fullname,
        code
      ...
      const errMsg = err.response?.data?.detail || "OAuth login failed. Please try again.";
      setError(errMsg);
      throw new Error(errMsg);
    }
  };
